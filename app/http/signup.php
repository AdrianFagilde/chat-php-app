...
if (
  ...
  $data = 'name=' . $name . '&username=' . $username . '&gender=' . $gender . '&birthDate=' . $birthDate;
    ...
    if ($stmt->rowCount() > 0) {
      ...
      if (isset($new_img_name)) {
        ...
        $sql = "INSERT INTO users
                  (name, username, password, birthDate, gender, p_p)
                  VALUES (?,?,?,?,?,?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$name, $username, $password, $birthDate, $gender, $new_img_name]);
      } else {
        # inserting data into database
        $sql = "INSERT INTO users
                  ...
                  VALUES (?,?,?,?,?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$name, $username, $password, $birthDate, $gender]);
      }
